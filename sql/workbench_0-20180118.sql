...
INSERT INTO `wb_user` VALUES (1,'*\ZÃÂÃÂ.ÃÂ\Ã',4,'ntorvez','ntorvez',4,'Ninja Torvez','blank.png','',0,1,'2017-01-29 20:12:24','2018-01-19 02:28:06');
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vw_contentlist` AS select `wb_menuitems`.`ID` AS `itemId`,`wb_menuitems`.`menuId` AS `menuId`,`wb_menuitems`.`menuType` AS `menuType`,`wb_content`.`ID` AS `contentId`,`wb_content`.`lang` AS `lang`,`wb_content`.`defaultParentId` AS `defaultParentId`,`wb_content`.`permalink` AS `permalink`,`wb_content`.`title` AS `title`,`wb_menuitems`.`sequence` AS `sequence`,`wb_content`.`status` AS `status`,`wb_content`.`target` AS `target`,`wb_content`.`galleryImage` AS `galleryImage`,`wb_content`.`pageType` AS `pageType`,`wb_content`.`ownerId` AS `ownerId`,`wb_content`.`authorFullName` AS `authorFullName`,`wb_content`.`authorLink` AS `authorLink` from (`wb_menuitems` join `wb_content`) where (`wb_menuitems`.`contentId` = `wb_content`.`ID`) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2018-01-18 22:25:11
